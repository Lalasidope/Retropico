...
BASE_COOLDOWN_AFTER_COMBAT = 2
MAX_COMBAT_START_WAIT = 3
script_running = True
script_paused = False
...
def get_random_sleep(base_time):
    return base_time * random.uniform(1, 2)

def get_random_offset(max_offset=2):
...
# ============ DÉTECTION D'IMAGE ============
def detect_and_click(resource_path, region, threshold=0.70, do_click=False, right_click=False, offset=(2, 0), no_offset=False):
    ...
    if max_val >= threshold:
        ...
        if do_click:
            ...
            pyautogui.moveTo(abs_x, abs_y, duration=random.uniform(0.5, 0.8))
            if right_click:
                pyautogui.click()
            else:
                pyautogui.click()
...
def action_placement():
    ...
    pyautogui.moveTo(855,681)
    time.sleep(1)
    pyautogui.click(855,681)
    time.sleep(1)
    pyautogui.moveTo(1726,484)
    pyautogui.press("f1")
